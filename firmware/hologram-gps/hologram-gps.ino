...
void setup()  
{
    ...
    SerialUSB.println("Simple GPS Asset Tracker");
...
void loop() {
  ...
  while(SerialCloud.available()) {
   ...
   if (gpsOutputTimer > millis())  gpsOutputTimer = millis();
}
...
void handleGPS() {
    ...
    if (millis() - gpsOutputTimer > GPS_SEND_TIME) { 
        ...
        if (GPS.fix) {
            ...
            String loc;
            char coordbuf[16];
            dtostrf(GPS.longitudeDegrees, 0, 5, coordbuf);
            loc.concat(coordbuf);
            loc.concat(", ");
            dtostrf(GPS.latitudeDegrees, 0, 5, coordbuf);
            loc.concat(coordbuf);
            String ret = "{\"coords\": [";
            ret.concat(loc);
            ret.concat("]}");
